...
This is a Magento 2 module template that enables you to customize Hyvä Checkout seamlessly. It adds a custom Webpack configuration, so that you can extend upon the React sources of the original Hyva Checkout module. In effect, this allows for kind of a parent/child theming mechanism, not for the entire Hyva theme, but only for the specific React sources in your custom checkout.
...
Currently, there are two ways to approach this issue. You can fork Hyvä Checkout and use the forked version in your project. You can now apply the customization to the forked version. Another approach would be setup Hyvä Checkout in the `app/code/Hyvä/Checkout` directory. In both cases, it will be difficult to get the updates in the original Hyvä Checkout repository. You need to manually port the changes.
...
Please remember this is a template. So you can name this module as you may like and there is no need for going with the given name `Hyva_CheckoutExample`. If you have a different name for this template then, you need to update the module name at least in below files.
...
## Credits
The brain behind this idea is [**Jisse Reitsma**](https://twitter.com/jissereitsma). You can find his original work here: [**Yireo_ExampleHyvaCheckout**](https://github.com/yireo-training/Yireo_ExampleHyvaCheckout).
